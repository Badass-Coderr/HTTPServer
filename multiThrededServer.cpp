...
void client_handler_thread(int client_socket) {
    // The handler will automatically close the socket in its destructor
    RequestHandler handler(client_socket);
    handler.process_request();
    std::cout << "✅ Connection handled and thread finished." << std::endl;
}
...
void Server::run() {
    ...
    while (true) {
        ...
        std::thread(client_handler_thread, client_socket).detach();
    }
}
